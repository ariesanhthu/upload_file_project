          ...
          {value?.map(({ file, progress }, index) => (
            <div key={index} className={variants.image + ' aspect-square'}>
              ...
              {imageUrls[index] && !disabled && progress === 'COMPLETE' && (
                ...
                >
                  <div className="flex h-5 w-5 cursor-pointer items-center justify-center rounded-md border border-solid bg-white transition-all duration-300 hover:h-6 hover:w-6 dark:border-gray-400 dark:bg-black">
                    <X
                      className=" dark:text-gray-400 text-red-500"
                      width={16}
                      height={16}
                    />
                  </div>
